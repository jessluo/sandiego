...
# delineate the front with cluster analysis
# tried clustering methods to determine the frontal region but this is not really working that well
# 
# # initialize
# phy$front <- NA
# 
...
# # create a subset of the phy data
# phy1 <- phy[which(phy$transect==1),which(names(phy) %in% c("salinity", "oxygen", "temp"))]
# phy1 <- phy1[seq(1, nrow(phy1), 10),]
# # select only 1 out of 100 data points
# # phy1temp <- phy1[seq(1, nrow(phy1), 50),]
# # 
# # phydist <- dist(phy1temp[,2:5], method="euclidean")
# # 
# # phy1clust <- hclust(phydist,method="ward")
# # plot(phy1clust, labels=phy1temp$front, cex=0.3)
# # 
# # rect.hclust(phy1clust, k=5, border="red")
# # groups <- cutree(phy1clust, 3)
# # table(groups)
# 
# # scale to a constant standard deviation
# phy1s <- scale(phy1)
# 
# # calculate distance matrix - euclidean
# phydist <- dist(phy1s, method="euclidean")
# phydist <- dist(phy1, method="euclidean")
# 
# 
...
# plot(phy1clust)
# 
# phy1sdf <- as.data.frame(phy1s)
# phy1sdf$front <- phy[which(phy$transect==1),"front"]
# plot(phy1clust, labels=phy1sdf$front, cex=0.3)
# 
# groups <- cutree(phy1clust, 5)
# indexc1 <- as.numeric(names(groups[which(groups==1)]))
# indexc2 <- as.numeric(names(groups[which(groups==2)]))
# indexc3 <- as.numeric(names(groups[which(groups==3)]))
# indexc4 <- as.numeric(names(groups[which(groups==4)]))
# indexc5 <- as.numeric(names(groups[which(groups==5)]))
# 
# 
# phy1 <- phy[which(phy$transect==1),which(names(phy) %in% c("salinity", "oxygen", "temp", "long", "cast", "depth"))]
# phy1 <- phy1[seq(1, nrow(phy1), 10),]
# phy1$cluster <- NA
# phy1[indexc1, "cluster"] <- 1
# phy1[indexc2, "cluster"] <- 2
# phy1[indexc3, "cluster"] <- 3
# phy1[indexc4, "cluster"] <- 4
# phy1[indexc5, "cluster"] <- 5
# 
# 
# ggplot(data=phy1) + geom_point(aes(x=long, y=-depth, colour=factor(cluster)))
